...
def get_date_range():
    ...
    root = Tk()
    root.withdraw()  # 隐藏主窗口
    date_range = simpledialog.askstring("输入日期范围", "请输入日期范围 (格式: YYYY/MM/DD-YYYY/MM/DD):")
    return date_range
...
def process_files(file_paths, date_range):
    """处理文件并保存结果"""
    # 解析日期范围
    ...
    for file_path in file_paths:
        ...
        for city, group_data in grouped:
            ...
            for week_str, data in zip(week_strings, split_data):
                ...
                # 文件名以周区间命名
                sanitized_week = week_str.replace("/", "-")
...
def main():
    ...
    if not files:
        ...
        if not date_range:
            ...
            process_files(files, date_range)
